...
command! -range -nargs=+ SubCase execute substitute(<q-args>, '\(\S\+\)\s\+\(\S\+\).*', 's/\1/\\=KeepCase(submatch(0), "\2")/gi', '')
command! -range -nargs=+ S execute substitute(<q-args>, '/\([^/]*\)/\([^/]*\)/\([^/]*\)', '<line1>,<line2>s/\1/\\=KeepCase(submatch(0), "\2")/i\3', '')
command! -range -nargs=+ SS execute substitute(<q-args>, ',\([^,]*\),\([^,]*\),\([^,]*\)', '<line1>,<line2>s,\1,\\=KeepCase(submatch(0), "\2"),i\3', '')
